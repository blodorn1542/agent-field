...
function createFieldReader({ adapters, defaultProvider }) {
  ...
  const api = {
     ...
     *
     * `since` is the argument to reach for. POM offers no date-range filter, so
     * the adapter serves a window by sorting newest-first and stopping early -
     * which means a windowed read costs a few pages and an unwindowed one walks
     * the company's whole history. On the Elite Pools tenant that is the
     * difference between 2 requests and 200.
     */
    async getServiceReports(args = {}) {
      const { since, until, siteIds, serviceTypeIds, flag, pageCap, pageSize } = args;
      return read(args, (a, t) => a.reads.getServiceReports(t, {
